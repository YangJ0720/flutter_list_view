...
class _HomePageState extends State<HomePage> {
  ...
  void _loadData() {
    List<TaskModel> list = [];
    list.add(
      TaskModel('去打架', DateTime(2021, 5, 1, 15, 30, 0), true, false),
    ); // 置顶
    list.add(
      TaskModel('去打球', DateTime(2021, 12, 30, 18, 0, 0), true, false),
    ); // 置顶
    list.add(
      TaskModel('去洗澡', DateTime(2021, 12, 11, 10, 0, 0), false, false),
    ); // 已过期
    list.add(
      TaskModel('去洗碗', DateTime(2022, 1, 1, 8, 0, 0), false, false),
    ); // 已过期
    list.add(
      TaskModel('去旅游', DateTime(2022, 10, 1, 8, 0, 0), false, false),
    ); // 其他日期
    // list.add(
    //   TaskModel('吃饺子', DateTime(2021, 12, 21, 20, 0, 0), false, true),
    // ); // 已完成// 置顶
        ...
        child: StreamBuilder<ExpandGroupModel>(
          builder: (_, snapshot) {
            ...
            return ExpandListView(
              model,
              (value) {
                _controller.sink.add(ExpandGroupDateModel.build(value.list));
              },
              (value) {
